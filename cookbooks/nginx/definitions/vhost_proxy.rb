define :nginx_vhost_proxy, :log_path => "/var/log/nginx/", :root => nil, :ssl => false, :ssl_only => false, :listen => nil, :upstream => [], :auth_basic => nil, :auth_basic_user_file => nil do
  ...
  template "#{node[:nginx][:conf_dir]}/sites-available/#{name}.conf" do
    ...
    variables(
      ...
      :ssl_cert             => params[:ssl_cert] || "/etc/ssl/#{node.fqdn}.crt",
      :ssl_cert_key         => params[:ssl_cert_key] || "/etc/ssl/#{node.fqdn}.key",
