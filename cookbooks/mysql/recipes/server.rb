...
node[:mysql][:server_id] ||= node[:ipaddress].gsub('.','')
...
# save node data after writing the MYSQL root password
ruby_block "save node data" do
  block do
    node.save
  end
...
monit_process :mysqld do
  executable "/usr/sbin/service mysql"
  pid_file "/var/run/mysqld/mysqld.pid"
end

(node[:rails_apps] || []).each do |app_name|
  # Let's make sure that we are giving our root user a local ip access
  mysql_user "Create root@localip" do
    username  "root"
    host      node.ipaddress
